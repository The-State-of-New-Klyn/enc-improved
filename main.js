    ...
    function fromBase85(str) {
        // Проверка на пустую или нестроковую строку
        ...
        // Проверяем, что последний символ – цифра 0-4 (паддинг)
        const lastChar = str.slice(-1);
        if (!/^[0-4]$/.test(lastChar)) {
            throw new Error('Некорректный паддинг в Base85 строке');
        }
        const padding = parseInt(lastChar, 10);
        const b85Data = str.slice(0, -1);
        if (b85Data.length % 5 !== 0) {
            throw new Error('Неверная длина Base85 данных (должна быть кратна 5)');
        }
        const words = [];
        for (let i = 0; i < b85Data.length; i += 5) {
            ...
            for (let j = 0; j < 5; j++) {
                ...
                if (code === -1) {
                    throw new Error(`Недопустимый символ '${b85Data[i + j]}' в Base85`);
                }
                val = val * 85 + code;
            }
            words.push(val | 0);
        ...
        if (sigBytes < 0) {
            throw new Error('Паддинг превышает размер данных');
        }
    ...
    async function hideToPNG() {
        ...
        try {
            const blob = await embedTextToPNG(file, resultText);
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
    ...
    async function extractAndDecrypt() {
        ...
        try {
            const extracted = await extractTextFromPNG(file);
            console.log('Извлечено (сырое):', extracted);
            if (!extracted) {
                alert('Не удалось извлечь текст из изображения (возможно, нет скрытых данных).');
                return;
            }
